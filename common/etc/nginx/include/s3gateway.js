...
import aws_sig from "./aws_sig.js";
...
function s3auth(r) {
    ...
    const credentials = aws_sig.readCredentials(r);
    if (sigver == '2') {
        signature = signatureV2(r, bucket, credentials);
    } else {
        let req = _s3canonicalReqParams(r, bucket, server);
        signature = aws_sig.signatureV4(r, NOW, region, SERVICE,
...
async function fetchCredentials(r) {
    ...
    try {
        current = aws_sig.readCredentials(r);
    } catch (e) {
    ...
    try {
        aws_sig.writeCredentials(r, credentials);
    } catch (e) {
